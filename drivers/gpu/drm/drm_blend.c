...
/*
 * DRM 混合/透明度属性系统 - 中文注释补充
 *
 * 本文件实现了 DRM 核心平面（plane）合成模型的标准属性，
 * 包括基本合成属性和合成相关的扩展属性。
 *
 * 基本合成模型：
 *   - 源矩形（SRC）：在 framebuffer 内的逻辑像素区域，支持亚像素精度
 *   - 目标矩形（CRTC）：CRTC 可见区域内的像素对齐目标区域
 *   - 通过缩放将源映射到目标
 *
 * 扩展平面属性：
 *   1. alpha：平面全局透明度（0 = 完全透明，0xffff = 完全不透明）
 *   2. rotation：旋转和镜像（0/90/180/270 度 + 反射）
 *   3. zpos：Z 轴顺序（控制平面叠加次序）
 *   4. pixel blend mode：像素混合模式（None/Pre-multiplied/Coverage）
 *   5. SCALING_FILTER：缩放滤波器选择
 *
 * CRTC 级扩展属性：
 *   - BACKGROUND_COLOR：背景色（所有平面之下的颜色）
 *
 * 这些属性通过一系列 drm_plane_create_*_property() 函数
 * 由驱动程序在初始化时注册。
 */
 ...
 */
/*
 * 中文说明：为平面创建全局透明度（alpha）属性。
 * 值范围 0（完全透明）到 0xffff（完全不透明）。
 * 与像素 alpha 值结合使用。framebuffer 中的像素值
 * 不应预先乘以全局 alpha 值。
 */
 ...
 */
/*
 * 中文说明：为平面创建旋转/镜像属性。用户空间可通过位掩码
 * 组合旋转角度（0/90/180/270）和镜像（X/Y 轴反射）。
 * 注意：180 度旋转等价于同时沿 X 和 Y 轴反射。
 * 驱动可用 drm_rotation_simplify() 归一化属性值。
 */
 ...
 */
/*
 * 中文说明：将旋转/镜像值归一化为硬件支持的形式。
 * 利用 180 度旋转等价于同时沿 X 和 Y 反射的特性，
 * 将不支持的反射标志转换为等效的旋转组合。
 * 调用者需检查返回值是否仍在支持的范围内。
 */
 ...
 */
/*
 * 中文说明：为平面创建可变的 Z 轴顺序（zpos）属性。
 * 用户空间可调整平面的叠加次序，值越小越靠下（背景侧）。
 * 驱动应在 atomic_check 中调用 drm_atomic_normalize_zpos()
 * 计算归一化 zpos。通常 min=0，max=CRTC 上平面数-1。
 */
 ...
 */
/*
 * 中文说明：创建不可变的 Z 轴顺序属性。用于硬件固定了
 * 平面叠加顺序的场景（如背景层始终在最下层、光标始终在最上层）。
 * 用户空间只能读取 zpos 值，不能修改。
 */
 ...
 */
/*
 * 中文说明：计算原子状态中所有 CRTC 上平面的归一化 zpos 值。
 * 对每个 CRTC，按 zpos（相同则按 plane id）排序平面，
 * 分配从 0 开始的连续值给 normalized_zpos。
 * 驱动应在 atomic_check() 中调用此函数。
 */
 ...
 */
/*
 * 中文说明：为平面创建像素混合模式（pixel blend mode）属性。
 * 支持三种混合模式：
 *   - None：忽略像素 alpha，仅使用平面 alpha
 *   - Pre-multiplied：像素已预乘 alpha
 *   - Coverage：标准 alpha 混合（像素未预乘）
 * 必须支持 Pre-multiplied 模式以保证向后兼容。
 */
 ...
 */
/*
 * 中文说明：为 CRTC 挂载背景色属性。背景色填充在所有平面
 * 之下的全屏图层，用于未被任何平面覆盖的像素区域。
 * 默认为不透明黑色，接受 DRM_ARGB64_PREP() 生成的 64 位 ARGB 值。
 * 背景色会经过 CRTC 的 degamma/CSC/gamma 变换。
 */
