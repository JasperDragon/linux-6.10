/*
 * drm_drv.c — DRM 设备驱动框架 (设备生命周期管理)。
 *
 * 负责 DRM 设备的核心生命周期:
 *   - drm_dev_init(): 分配并初始化 drm_device
 *   - drm_dev_register(): 注册字符设备节点 (/dev/dri/cardN, /dev/dri/renderDN)
 *   - Minor 分配: 主节点 0-63, 渲染节点 128-191, 加速器节点独立 major
 *   - drm_open()/drm_release(): 文件操作, 创建 per-fd 的 drm_file 上下文
 *
 * Created: Fri Jan 19 10:48:35 2001 by faith@acm.org
 * Copyright 2001 VA Linux Systems, Inc.
 */
