...
/*
 * ============================================================================
 * xhci.h — XHCI (eXtensible Host Controller Interface) 主控驱动头文件
 * ============================================================================
 *
 * XHCI 是 USB 3.x 的标准主机控制器接口。本头文件定义 XHCI 硬件寄存器布局、
 * 内存数据结构 (TRB/Ring/Context) 以及驱动的软件抽象。
 *
 * 【核心数据结构】
 *   TRB (Transfer Request Block): 16 字节传输描述符, XHCI 的基本传输单元
 *   Transfer Ring: 循环链表, 驱动生产 TRB, HC 消费 TRB
 *   Event Ring: HC 写入完成事件, 驱动读取
 *   Device Context: 每个 USB 设备的 slot 上下文 (含 EP 上下文数组)
 *   Doorbell Array: 驱动写 Doorbell 寄存器通知 HC 有新的工作
 *
 * 【寄存器组】
 *   Capability Regs (CAP): HC 能力, 只读
 *   Operational Regs (OP): 运行时控制 (USBCMD, USBSTS, PAGESIZE, ...)
 *   Runtime Regs (RT): 中断管理 (IMAN, IMOD, ERSTSZ, ...)
 *   Doorbell Array (DB): 端点通知
 *   Extended Capabilities (EXT): USB3/USB2 协议端口
 *
